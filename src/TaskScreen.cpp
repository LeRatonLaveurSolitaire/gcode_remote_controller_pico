...
void TaskScreen(void *pvParameters){
  ...
  for (;;) {// A Task shall never return or exit.
    ...
    do
    {
        if (system_state_local.selected_axis == AXIS_X)
        {
            u8g2.setDrawColor(0);
        }
        else
        ...
        if (system_state_local.selected_axis == AXIS_Y)
        {
            u8g2.setDrawColor(0);
        }
        else
        ...
        if (system_state_local.selected_axis == AXIS_Z)
        {
            u8g2.setDrawColor(0);
        }
        else
        ...
        u8g2.setCursor(66, 16 * 3 - 2);
        u8g2.print("Step");
        system_state_local.step_size < 0.5 ? sprintf(msg, " %.2f", system_state_local.step_size) : sprintf(msg, "  %2.0f", system_state_local.step_size);
        u8g2.print(msg);

        char axis[] = {'X','Y','Z'};
        u8g2.setCursor(66, 16 * 4 - 2);
        sprintf(msg, "Select %c", axis[system_state_local.selected_axis]);
        u8g2.print(msg);
